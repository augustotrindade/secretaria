	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				if(!in_array($this->ext($fileName),$this->_allowed)){
					$this->error("Tipo de arquivo não disponível.");
				} else { 
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							...
							if (move_uploaded_file($fileTmp, $output)) {
								...
								$this->error("Não é possível mover o arquivo '$fileName' para '$destination'");
							}
						} else {
							// -- gd lib check
							if (function_exists("imagecreatefromjpeg")) {
								...
								if (isset($rules['type']) && isset($rules['size'])) {
									...
									$this->error("Parâmetros \"rules\" inválidos");
								}
							} else {
								$this->error("Biblioteca GD não esta instalada");
							}
						}
					} else {
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			if ($write) {
				...
				$this->error("Não foi possivel gravar este arquivo no servidor, entre em contato com o NTI.");//"Could not write " . $this->_name . " to " . $this->_destination
			}
		}
