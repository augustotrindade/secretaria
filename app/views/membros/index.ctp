...
<?php $session->flash(); ?>
<?php echo $form->create('Membro',array('action'=>'index')); ?>
		...
		<input type="submit" value="Pesquisar"> <input type="button" value="Novo" onclick="javascript:window.location.href='<?php echo $html->url(array('controller'=>'membros','action'=>'add')) ?>'">
<?php echo $form->end() ?>
<?php echo $paginator->options(array('url' => array('nome'=>$nome,'id'=>$id,'congregacao_id'=>$congregacao,'situacao'=>$situacao)));?>
...
<?php endforeach; ?>
	...
	<tr>
		<td colspan="5"><?php echo $form->submit('Imprimir') ?></td>
	</tr>
</table>
</div>
